import {
  ...
  const CreateUserDrawer: React.FC<CreateUserDrawerProps> = ({
    ...
    const handleRoleChange = (selectedRoleNames: string[]) => {
      ...
      const newPermissions = selectedRoles.flatMap((r) =>
      ...
      const validRevoked = currentRevoked.filter((p: string) =>
    ...
    const permissionOptions = useMemo(
      () => permissions.map((p) => ({ label: p, value: p })),
      [permissions]
    );
  
    return (
      <Drawer title="Tạo người dùng mới" 
      width={480} 
      onClose={onClose} 
      open={open}
        ...
        {loading ? (
          ...
          <Form layout="vertical" form={form} onFinish={handleSubmit}>
            ...
            <Form.Item
              ...
              label="Số điện thoại"
              rules={[
                { pattern: /^[0-9]{10}$/, message: "Phải đủ 10 chữ số" },
              ]}
            >
              <Input />
            </Form.Item>
